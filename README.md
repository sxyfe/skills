...
| Skill | 说明 | MCP 服务 | 安装 |
| --- | --- | --- | --- |
| [flight-monitor-agent](skills/flight-monitor-agent/) | 航班实时查价、精简/全量穷举、暖色 HTML 分页报告 | RollingGo-Flight | `npx skills add sxyfe/skills@flight-monitor-agent -g -y` |
| [hotel-watch-agent](skills/hotel-watch-agent/) | 全球酒店价格监控、A/B 预算、组合触发、cron+Agent 定时告警 | RollingGo-Hotel | `npx skills add sxyfe/skills@hotel-watch-agent -g -y`（待发布） |
...
| 源码位置 | 本仓库 `skills/flight-monitor-agent/` | 本仓库 `skills/hotel-watch-agent/` |
...
两个 Skill 源码均在本仓库 `skills/` 目录。本地调试时，可将目录链接到所用 Agent 的 Skills 路径：
...
# Cursor
ln -s "$REPO_ROOT/skills/flight-monitor-agent" ~/.cursor/skills/flight-monitor-agent
ln -s "$REPO_ROOT/skills/hotel-watch-agent" ~/.cursor/skills/hotel-watch-agent

# Claude Code（项目级示例）
mkdir -p .claude/skills
ln -s "$REPO_ROOT/skills/flight-monitor-agent" .claude/skills/flight-monitor-agent
```
